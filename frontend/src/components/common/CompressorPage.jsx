...
const CompressorPage = ({ acceptedFormats, title, description, settingsComponent: SettingsComponent, defaultSettings }) => {
  ...
  // Initialize files from location state (if any)
  React.useEffect(() => {
  ...
  const startCompression = async () => {
    ...
    try {
      const results = await fileProcessingService.batchCompress(filesToProcess);
      ...
      // The interceptor will skip redirect for /api/compress, so we don't need to handle that here.
      const errorMessage = error.response?.data?.message || 'Compression failed.';
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        {hasFiles && (
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-200 dark:border-gray-700 mb-8">
            <div className="p-6">
              <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-4">Files to Compress ({totalFiles})</h3>
            ...
            {isAuthenticated ? (
              ...
              >
                {isProcessing ? (
                  ...
                  <span>Compress {files.filter(f => f.status === 'ready').length} File{files.length > 1 ? 's' : ''}</span>
                )}
              </button>
            ) : (
              <button
