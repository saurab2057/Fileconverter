...
const CompressorPage = ({ acceptedFormats, title, description, settingsComponent: SettingsComponent, defaultSettings }) => {
  ...
  const startCompression = async () => {
    ...
    try {
      ...
      console.error('An error occurred during the batch compression request:', error);
  ...
  const totalFilesToCompress = files.length;
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 transition-colors duration-300">
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <FileUploader
          ...
          subtitle={`Drop your ${acceptedFormats.map(f => f.toUpperCase()).join(', ')} files here or click to browse (Max ${MAX_ALLOWED_FILES} files)`}
          maxFileSize="100MB"
        ...
        {files.length > 0 && (
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-200 dark:border-gray-700 mb-8">
            <div className="p-6">
              <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-4">Files to Compress ({files.length})</h3>

              {isCompressing && (
                <p className="text-sm text-blue-600 dark:text-blue-400 mb-4">
                  Processing {completedOrErrorFiles} of {totalFilesToCompress} files...
                </p>
              )}
        ...
        {files.length > 0 && !compressionComplete && (
            ...
            {isAuthenticated ? (
              ...
              >
                {isCompressing ? (
                  ...
                  <span>Compress {files.length} File{files.length > 1 ? 's' : ''}</span>
                )}
              </button>
            ) : (
              <button
        ...
        {/* BUG FIX: show "Compress More" whenever complete, not just on success —
            previously `files.some(f => f.status === 'completed')` meant users with
            all-error batches were stuck with no way to reset. */}
        {compressionComplete && (
          <div className="text-center mb-8">
            <button
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4" onClick={() => setShowAuthModal(false)}>
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl max-w-md w-full p-8 relative" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="text-center mb-6">
              ...
              <h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-2">Sign in to compress files</h2>
              <p className="text-gray-600 dark:text-gray-400">Your files are ready! Sign in to unlock compression.</p>
            </div>
            <div className="space-y-3">
