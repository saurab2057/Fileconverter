// src/components/common/FileProcessingPage.jsx
...
/**
 * Unified Page Component for both Compression and Conversion workflows.
 * @param {string} mode - 'compress' or 'convert'
 * @param {array} acceptedFormats - Array of formats (used in compress mode)
 * @param {string} fromFormat - Source format (used in convert mode)
 * @param {string} toFormat - Target format (used in convert mode)
 * ... (other props)
 */
const FileProcessingPage = ({ 
  ...
  const {
    files, uploadLimitExceeded, addFiles, removeFile, updateFileSettings,
    updateFileStatus, updateAllStatus, resetBatch, hasFiles, canProcess,
    maxAllowedFiles, formatFileSize,
  } = useFileBatch(defaultSettings);

  // ✅ FIXED: Added proper dependencies to prevent silent failures on route state changes
  useEffect(() => {
  ...
  }, [location.state, files.length, addFiles, updateFileSettings]);

  // Dynamic text based on mode
  const actionWord = mode === 'compress' ? 'Compress' : 'Convert';
  const actionWordLower = actionWord.toLowerCase();
  
  // Determine formats to show in UI
  const displayFormats = mode === 'convert' 
  ...
  const startProcessing = async () => {
    ...
    try {
      // ✅ Dynamic API call based on mode
      const results = mode === 'compress' 
        ? await fileProcessingService.batchCompress(filesToProcess)
        : await fileProcessingService.batchConvert(filesToProcess, toFormat);
      ...
      const errorMessage = error.response?.data?.message || `${actionWord} failed.`;
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 transition-colors duration-300">
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <FileUploader
          acceptedFormats={mode === 'convert' ? [fromFormat] : acceptedFormats}
          title="Choose Files"
          subtitle={`Drop your ${displayFormats.join(', ')} files here`}
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-200 dark:border-gray-700 mb-8">
            <div className="p-6">
              <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-4">
                Files to {actionWord} ({totalFiles})
              </h3>
                    ...
                    <div className="flex items-center space-x-1 shrink-0">
                      ...
                      {file.status === 'completed' && (
                        <div className="flex items-center space-x-2">
                          <a href={file.downloadUrl} download className="bg-green-600 hover:bg-green-700 text-white p-2 sm:px-3 sm:py-2 rounded-lg text-sm font-medium flex items-center">
              ...
              >
                {isProcessing ? (
                  <>
                    ...
                    <span>{actionWord}ing</span>
                  </>
                ) : (
                  <span>{actionWord} {readyFilesCount} File{readyFilesCount !== 1 ? 's' : ''}</span>
                )}
              </button>
            ) : (
              <button
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4" onClick={() => setShowAuthModal(false)}>
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl max-w-md w-full p-8 relative" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="text-center mb-6">
              ...
              <h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-2">Sign in to {actionWordLower} files</h2>
              <p className="text-gray-600 dark:text-gray-400">Your files are ready! Sign in to unlock {actionWordLower}.</p>
            </div>
            <div className="space-y-3">
