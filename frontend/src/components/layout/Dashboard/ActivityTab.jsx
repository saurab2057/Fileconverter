...
import { FileText, FileImage, FileVideo, FileAudio,
         RotateCw, AlertTriangle, ChevronLeft, ChevronRight } from 'lucide-react';
    ...
    <div className="max-w-3xl">
      <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl overflow-hidden shadow-sm">
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-200 dark:border-gray-700">
          <div>
            ...
            <p className="text-[12px] text-gray-400 dark:text-gray-500 mt-0.5">
              {total > 0 ? `${total.toLocaleString()} file${total !== 1 ? 's' : ''} processed` : 'No history yet'}
            </p>
          </div>
        ...
        {isLoading ? (
          ...
          <div className={`divide-y divide-gray-100 dark:divide-gray-700/80 transition-opacity duration-150 ${isFetching ? 'opacity-60' : 'opacity-100'}`}>
            {history.map(item => {
              const IconComp = getFileIcon(item.format);
              return (
                <div key={item._id} className="flex items-center gap-3 px-5 py-3 hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors">
