...
const ResetPasswordPage = () => {
  ...
  const exchangeTokenForSession = async (token) => {
    ...
    try {
      ...
      // Remove the one-time reset token from the browser URL after
      // successful validation so it is not unnecessarily exposed.
      setSearchParams({}, { replace: true });

      // The password form is only allowed after successful validation.
      ...
      // Explicitly keep the reset session in an unverified state.
      setIsTokenExchanged(false);

      // Remove the invalid token from the browser URL.
      setSearchParams({}, { replace: true });
    } finally {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      if (err.response?.status === 401) {
        ...
        // The reset session is no longer valid, so immediately
        // return the component to the unverified state.
        setIsTokenExchanged(false);
        setSearchParams({}, { replace: true });
      } else {
        toast.error(
  ...
  // Never allow the password form to render while the reset token
      ...
      <div className="min-h-screen bg-gradient-to-br from-black via-gray-900 to-emerald-900 flex items-center justify-center p-4">
        <div className="w-full max-w-sm mx-auto bg-white/10 backdrop-blur-md rounded-2xl shadow-xl border border-white/20 p-6 text-center">
          ...
          <p className="text-gray-300">
            This link will self-destruct after use for your security
          </p>
        </div>
      </div>
    );
  ...
  // The password form must only be reachable after successful
  // reset-token validation and session creation.
  if (!isTokenExchanged) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-black via-gray-900 to-emerald-900 flex items-center justify-center p-4">
        <div className="w-full max-w-sm mx-auto bg-white/10 backdrop-blur-md rounded-2xl shadow-xl border border-white/20 p-6 text-center">
          <h2 className="text-xl font-bold text-white mb-4">
            🔒 Secure Reset Session Required
          </h2>
  ...
  // This form can only be rendered after validateResetToken()
  // has successfully exchanged the reset token for a secure session.
  return (
    <div className="min-h-screen bg-gradient-to-br from-black via-gray-900 to-emerald-900 flex items-center justify-center p-4">
          ...
          <div className="relative z-10">
            ...
            >
              ...
              <p className="text-xs text-gray-400 text-center mt-2">
                🔒 Your reset session expires in 5 minutes. All previous
                sessions will be revoked after reset.
              </p>
            </form>
          </div>
        </div>
