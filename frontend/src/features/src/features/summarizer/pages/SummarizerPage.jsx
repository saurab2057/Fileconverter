...
import { fileProcessingService } from '@/services/fileProcessingService'; // ✅ Use existing service
...
// ==========================================
const SummarizerPage = () => { // ✅ Renamed for consistency
  ...
  // ✅ IMPROVED: Truncate directly on change instead of using a useEffect loop
  ...
  const summarize = async () => {
    ...
    try {
      // ✅ IMPROVED: Use the centralized service instead of raw apiClient
      ...
      const fullText = response.data || response || ''; // Handle varying response structures
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="bg-white dark:bg-gray-800 rounded-b-xl border border-gray-200 dark:border-gray-700 p-4 flex items-center justify-between">
            ...
            <div className="flex gap-3">
              <label className="px-4 py-2 bg-gray-200 hover:bg-gray-300 dark:bg-gray-700 dark:hover:bg-gray-600 text-gray-800 dark:text-gray-200 rounded-lg cursor-pointer transition-colors flex items-center gap-2">
                ...
                <input type="file" accept=".pdf" onChange={(e) => e.target.files?.[0] && handleFileUpload(e.target.files[0])} className="hidden" disabled={isProcessing} />
              </label>
              <button
          ...
          <div className="bg-white dark:bg-gray-800 rounded-t-xl border border-gray-200 dark:border-gray-700 p-4 flex items-center justify-between">
            ...
            {summary && (
              <div className="flex gap-2">
                <button onClick={() => navigator.clipboard.writeText(summary)} className="px-3 py-1.5 text-sm bg-gray-200 hover:bg-gray-300 dark:bg-gray-700 dark:hover:bg-gray-600 text-gray-800 dark:text-gray-200 rounded-lg transition-colors">
                  Copy
                </button>
                <button onClick={handleDownload} className="px-3 py-1.5 text-sm bg-gray-200 hover:bg-gray-300 dark:bg-gray-700 dark:hover:bg-gray-600 text-gray-800 dark:text-gray-200 rounded-lg transition-colors flex items-center gap-2">
      ...
      {/* Auth Modal (Identical to your other pages, which is great for consistency) */}
      {showAuthModal && (
